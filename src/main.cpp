...
int main() {
    ...
    // --------------------
    GLFWwindow* window =
        glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Kinetic Sculpture", NULL, NULL);
    if (window == NULL) {
    ...
    // ------------------------------------
    Shader lightingShader("src/shaders/kinetic_sculpture.vs", "src/shaders/kinetic_sculpture.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // point lights
        for (int i = 0; i < sizeof(pointLightPositions) / sizeof(pointLightPositions); i++)
        {
        ...
        for (int i = 0; i < numCubes; i++) {
            ...
            float angle = (float)i / numCubes * 2.0f * glm::pi<float>() + currentFrame * rotationSpeed;
            float x = spiralRadius * cos(angle);
            float z = spiralRadius * sin(angle);
            float y = (float)i * spiralHeight - (numCubes * spiralHeight / 2.0f) + sin(currentFrame * oscillationSpeed + i) * 0.5f;

            model = glm::translate(model, glm::vec3(x, y, z));
            model = glm::rotate(model, angle + currentFrame * 2.0f, glm::vec3(0.0f, 1.0f, 0.0f));
            model = glm::scale(model, glm::vec3(0.3f)); // Smaller cubes

            // Set different colors for each cube
            glm::vec3 diffuseColor = glm::vec3(
                (sin(currentFrame * 0.5f + i * 0.1f) + 1.0f) / 2.0f,
                (cos(currentFrame * 0.7f + i * 0.15f) + 1.0f) / 2.0f,
                (sin(currentFrame * 0.9f + i * 0.2f) + 1.0f) / 2.0f
            );
            lightingShader.setVec3("material.diffuse", diffuseColor);
            lightingShader.setVec3("material.specular", 0.5f, 0.5f, 0.5f); // Consistent specular for all cubes
